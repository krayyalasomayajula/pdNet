...
-- Batch test:
local x, yt
if(opt.dataset == 'dbe') then
  x = torch.Tensor(opt.batchSize, opt.channels+1, opt.imHeight, opt.imWidth)
else
  x = torch.Tensor(opt.batchSize, opt.channels, opt.imHeight, opt.imWidth)
end
yt = torch.Tensor(opt.batchSize, opt.labelHeight, opt.labelWidth)
x = x:cuda()
yt = yt:cuda()

local edge
if(opt.dataset == 'dbe') then
  edge = torch.Tensor(opt.batchSize, opt.imHeight, opt.imHeight)
  edge = edge:cuda()
end
...
function test(testData, classes, epoch, trainConf, model, loss )
   ...
   if totalerr < testData.preverror then
      print('\27[32mBetter model with lower loss found\27[31m!!!\27[0m')
      filename = paths.concat(opt.save, 'model-'.. opt.year .. '.net')
      --print('==> saving model to '..filename)
      
      if(opt.model == 'models/decoder_pdNet.lua' or opt.model == 'models/pdNet.lua') then
      ...
      filename = paths.concat(opt.save, opt.year .. '_best-number.txt')
      local file = io.open(filename, 'w')
      file:write("----------------------------------------\n")
