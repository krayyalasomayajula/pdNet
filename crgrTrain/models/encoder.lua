...
if(paths.filep(opt.ptModel) == true) then -- pre-trained encoder CNN:
  print(string.format('\27[31m\27[4m\nLoading pre-trained encoder # \27[0m'))
  nn.DataParallelTable.deserializeNGPUs = 1
  model = torch.load(opt.ptModel)
  if torch.typename(model) == 'nn.DataParallelTable' then model = model:get(1) end
else -- Construct encoder CNN:
  print('\27[32m==> Construct Encoder Model\27[0m')
  model = nn.Sequential()
  ...
  function _augChNet()
    ...
    local sepRGBCh  = nn.ConcatTable()
    local redCh  = nn.Sequential():add(nn.Narrow(2, 1, 1)):add(nn.MulConstant(0.299, false))
    local grnCh  = nn.Sequential():add(nn.Narrow(2, 2, 1)):add(nn.MulConstant(0.587, false))
    local bluCh  = nn.Sequential():add(nn.Narrow(2, 3, 1)):add(nn.MulConstant(0.114, false))
    sepRGBCh:add(redCh)
    sepRGBCh:add(grnCh)
    sepRGBCh:add(bluCh)
    local gryRGBCh  = nn.Sequential()
    gryRGBCh:add(sepRGBCh):add(nn.CAddTable())
    
    local sepBGRCh  = nn.ConcatTable()
    local bCh  = nn.Sequential():add(nn.Narrow(2, 1, 1)):add(nn.MulConstant(0.114, false))
    local gCh  = nn.Sequential():add(nn.Narrow(2, 2, 1)):add(nn.MulConstant(0.587, false))
    local rCh  = nn.Sequential():add(nn.Narrow(2, 3, 1)):add(nn.MulConstant(0.299, false))
    sepBGRCh:add(bCh)
    sepBGRCh:add(gCh)
    sepBGRCh:add(rCh)
    local gryBGRCh  = nn.Sequential()
    gryBGRCh:add(sepBGRCh):add(nn.CAddTable())
    
    augNetInitBlock:add(allCh)
    augNetInitBlock:add(gryRGBCh)
    augNetInitBlock:add(gryBGRCh)
    
    local augNet = nn.Sequential()
    augNet:add(augNetInitBlock):add(nn.JoinTable(2))
    
    local augClrGrayAllBlock = nn.ConcatTable()
    
    augClrGrayAllBlock:add(augNet)
    augClrGrayAllBlock:add(allCh)
    
    return augClrGrayAllBlock    
  end
  
  function _augGrayNet()
  
    local augNetInitBlock = nn.ConcatTable()
    
    local grayCh1  = cudnn.SpatialConvolution(3, 1, 1, 1)
    local grayCh2  = cudnn.SpatialConvolution(3, 1, 1, 1)
    local grayCh3  = cudnn.SpatialConvolution(3, 1, 1, 1)
    
    augNetInitBlock:add(grayCh1)
    augNetInitBlock:add(grayCh2)
    augNetInitBlock:add(grayCh3)
  ...
  function _augChNet2()
    ...
    local onlyGrayBlock = nn.Sequential()
    local grayInitBlock = nn.ConcatTable()
    grayInitBlock:add(gryRGBCh)
    grayInitBlock:add(gryBGRCh)
    onlyGrayBlock:add(grayInitBlock):add(nn.JoinTable(2))
    
    local augClrGrayAllBlock = nn.ConcatTable()
    
    augClrGrayAllBlock:add(augNet)
    augClrGrayAllBlock:add(onlyGrayBlock)
  ...
  local augChNet = _augChNet()
  model:add(nn.Identity())
  local initial_block = nn.ParallelTable()
  initial_block:add(cudnn.SpatialConvolution(5, 13, 3, 3, 2, 2, 1, 1))
  
  --local augChNet = _augGrayNet()
  --model:add(augChNet)  
  --local initial_block = nn.ConcatTable(2)
  --initial_block:add(cudnn.SpatialConvolution(3, 13, 3, 3, 2, 2, 1, 1))
  
  --local augChNet = _augChNet2()
  --model:add(nn.Identity())
  --model:add(augChNet)
  --local initial_block = nn.ParallelTable()
  --initial_block:add(cudnn.SpatialConvolution(5, 14, 3, 3, 2, 2, 1, 1))
...
--local normHist = histClasses/histClasses:max()
...
for i = 1, #classes do
   if histClasses[i] < 1 or i == #classes then -- ignore unlabeled
      classWeights[i] = 0
   else
      --classWeights[i] = 1 / (torch.log(1.2 + normHist[i]))
      classWeights[i] = torch.pow(torch.log(1.2 + normHist[i]), -1)
   end
end
